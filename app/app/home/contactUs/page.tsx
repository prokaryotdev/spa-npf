    ...
    <PageShell title={t("Contact Us")} intro={contactUs.intro}>
      <section className="bg-white pb-16">
        <div className="dp-container">
          ...
          <p className="mt-3 max-w-[70ch] text-base text-dp-body">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            {contactUs.reach.lines.map((line) => (
              <div
                key={line.number}
                className="flex items-center gap-5 rounded-3xl bg-[#F4F8F6] p-6"
              >
                <span className="rounded-2xl bg-white p-4 text-dp-green-ink">
                  <PhoneIcon className="size-8" />
                </span>
                <div className="min-w-0">
                  <p className="font-secondary text-sm font-bold text-dp-ink">
                    {line.title}
                  </p>
                  <a
                    href={`tel:${line.number}`}
                    className="font-secondary text-4xl font-bold tabular-nums text-dp-green-deep transition-opacity hover:opacity-80"
                  >
                    {line.number}
                  </a>
          ...
          <div className="mt-6 rounded-3xl bg-white p-6 ring-1 ring-black/5">
            ...
            <p className="mt-2 max-w-[70ch] text-base text-dp-body">
              {contactUs.reach.signLanguage.subTitle}
            </p>
            <p className="mt-1 max-w-[70ch] text-sm text-dp-muted">
