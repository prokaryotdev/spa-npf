...
const EMAIL = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
/** UAE mobile, with or without spaces: +9715XXXXXXXX or 05XXXXXXXX. */
...
export default function PortalProfile() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!phone) next.phone = "Enter a mobile number.";
      ...
      next.phone = "Use a UAE mobile number, like 050 123 4567.";
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 py-4">
      ...
      <dd className="font-secondary text-sm font-bold text-dp-ink">{value}</dd>
    </div>
  );
}
