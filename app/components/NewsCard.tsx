...
export default function NewsCard({
  ...
  return (
    ...
    >
      <Link href={`/app/home/media/news/${item.slug}`} className="flex flex-col">
        {item.image ? (
        ...
        <p className="mt-2 text-sm leading-relaxed text-dp-body">{item.summary}</p>
        <span className="mt-3 text-sm font-medium text-dp-green">Read more</span>
      </Link>
    </article>
  );
}
