/**
 * Interface chrome: navigation, controls, form labels, empty and error
 * states — everything written into a component rather than into a content
 * file. Modern Standard Arabic as UAE government services write it.
 */
export const arChrome: Record<string, string> = {
  // --- header, footer, global ---------------------------------------------
  "Skip to main content": "تخطَّ إلى المحتوى الرئيسي",
  Home: "الرئيسية",
  "Main Menu": "القائمة الرئيسية",
  "Main menu": "القائمة الرئيسية",
  Main: "التنقل الرئيسي",
  "Open menu": "فتح القائمة",
  "Close menu": "إغلاق القائمة",
  Close: "إغلاق",
  Breadcrumb: "مسار التنقل",
  "Government of Dubai": "حكومة دبي",
  "Dubai Police": "شرطة دبي",
  "Dubai Police home": "الصفحة الرئيسية لشرطة دبي",
  "Dubai Police - Smart Secure Together": "شرطة دبي - أذكى، أأمن، معًا",
  "Together for a safer Dubai tomorrow. Report, apply, inquire and pay, and request support from Dubai Police.":
    "معًا من أجل دبي أكثر أمانًا غدًا. أبلغ، وتقدّم بطلبك، واستعلم وادفع، واطلب المساندة من شرطة دبي.",
  "Larger text": "تكبير النص",
  Search: "بحث",
  "Search Dubai Police": "البحث في موقع شرطة دبي",
  "Search Dubai Police services": "البحث في خدمات شرطة دبي",
  "Search services": "البحث في الخدمات",
  "Search for a service": "ابحث عن خدمة",
  "Search for a service, news or page": "ابحث عن خدمة أو خبر أو صفحة",
  "Find a service, a news story, an event or a page.":
    "ابحث عن خدمة أو خبر أو فعالية أو صفحة.",
  Suggestions: "الاقتراحات",
  Recent: "عمليات البحث الأخيرة",
  Clear: "مسح",
  "See all results for": "عرض كل النتائج عن",
  "Nothing matches": "لا توجد نتائج مطابقة لـ",
  ". Try a shorter word, or": ". جرّب كلمة أقصر، أو",
  "browse all services": "تصفّح جميع الخدمات",
  "Browse services": "تصفّح الخدمات",
  "Nothing matched. Try a shorter term, or browse the":
    "لا توجد نتائج. جرّب كلمة أقصر، أو تصفّح",
  Sitemap: "خريطة الموقع",
  "Every page on the Dubai Police website, in one list.":
    "جميع صفحات موقع شرطة دبي في قائمة واحدة.",
  "Most used": "الأكثر استخدامًا",
  "Popular services": "الخدمات الأكثر طلبًا",
  Services: "الخدمات",
  "services page": "صفحة الخدمات",
  Account: "الحساب",

  // --- footer --------------------------------------------------------------
  "Emergency Numbers": "أرقام الطوارئ",
  "Subscribe to our Newsletter": "اشترك في نشرتنا الإخبارية",
  "Stay updated with the latest news and announcements.":
    "ابقَ على اطلاع بآخر الأخبار والإعلانات.",
  "Email Address": "البريد الإلكتروني",
  Subscribe: "اشتراك",
  "Thanks — you are on the list.": "شكرًا لك — تم تسجيلك في القائمة.",
  "© 2026 Dubai Police General HQ. All Rights Reserved":
    "© 2026 القيادة العامة لشرطة دبي. جميع الحقوق محفوظة",
  "This site is monitored and maintained by Dubai Police.":
    "تتم مراقبة هذا الموقع وصيانته من قِبل شرطة دبي.",
  "The site is best viewed using IE11 and above, Mozilla Firefox, Safari and Chrome":
    "يُفضَّل تصفّح الموقع باستخدام إنترنت إكسبلورر 11 فأحدث، أو موزيلا فايرفوكس، أو سفاري، أو كروم",
  "Last modified Date: 11/09/2026": "تاريخ آخر تحديث: 11/09/2026",
  "Quick access toolbar": "شريط الوصول السريع",
  "Happiness Meter (opens in a new window)":
    "مؤشر السعادة (يفتح في نافذة جديدة)",
  "Customer Centers": "مراكز العملاء",
  "Contact Us": "اتصل بنا",
  "contact us": "اتصل بنا",
  "Download the Dubai Police App": "حمّل تطبيق شرطة دبي",
  "Services, updates, and alerts: right in your pocket.":
    "الخدمات والتحديثات والتنبيهات في متناول يدك.",
  "Available on": "متوفر على",
  "(opens in a new window)": "(يفتح في نافذة جديدة)",

  // --- generic actions -----------------------------------------------------
  "More Details": "تفاصيل أكثر",
  "See all": "عرض الكل",
  Add: "إضافة",
  Cancel: "إلغاء",
  Send: "إرسال",
  "Send another": "إرسال رسالة أخرى",
  Download: "تنزيل",
  "Try again": "حاول مرة أخرى",
  "Back to home": "العودة إلى الرئيسية",
  "Go to my account": "الانتقال إلى حسابي",
  "Save changes": "حفظ التغييرات",
  "Clear filters": "مسح عوامل التصفية",
  "Sort by": "الترتيب حسب",
  "Filter by category": "التصفية حسب الفئة",
  "Previous page": "الصفحة السابقة",
  "Next page": "الصفحة التالية",
  "Previous slide": "الشريحة السابقة",
  "Next slide": "الشريحة التالية",
  "Pause slideshow": "إيقاف العرض مؤقتًا",
  "Play slideshow": "تشغيل العرض",
  "Go to slide {n}": "الانتقال إلى الشريحة {n}",
  "Show previous {label} item": "عرض العنصر السابق في {label}",
  "Show next {label} item": "عرض العنصر التالي في {label}",
  "{name} logo": "شعار {name}",
  "Scroll to next section": "انتقل إلى القسم التالي",
  "Open in Maps": "الفتح في الخرائط",
  "Show the numbers": "عرض الأرقام",
  "On this page": "في هذه الصفحة",
  "Searching…": "جارٍ البحث…",

  // --- errors and empty states --------------------------------------------
  "Page not found": "الصفحة غير موجودة",
  "That page has moved or never existed. Here is the way back.":
    "هذه الصفحة انتقلت أو لم تكن موجودة أصلًا. من هنا تعود.",
  "Something went wrong": "حدث خطأ ما",
  "Something went wrong | Dubai Police": "حدث خطأ ما | شرطة دبي",
  "The site failed to load. Trying again often clears it.":
    "تعذّر تحميل الموقع. غالبًا ما تحل المحاولة مرة أخرى المشكلة.",
  "This page failed to load. Trying again often clears it.":
    "تعذّر تحميل هذه الصفحة. غالبًا ما تحل المحاولة مرة أخرى المشكلة.",
  "Downloads are not available in this rebuild":
    "التنزيلات غير متاحة في هذه النسخة التجريبية",

  // --- list controls -------------------------------------------------------
  "More services": "خدمات أخرى",
  "Open: {hours}": "ساعات العمل: {hours}",
  Location: "الموقع",
  Contact: "التواصل",
  "Search {noun}": "ابحث في {noun}",
  "Search {what}": "ابحث في {what}",
  "Filter by name": "التصفية بالاسم",
  "Showing {shown} of {total} {noun}": "عرض {shown} من {total} {noun}",
  "Showing {shown} of {total} requests": "عرض {shown} من {total} طلبًا",
  "Showing {shown} of {total} services": "عرض {shown} من {total} خدمة",
  "{shown} of {total}": "{shown} من {total}",
  "Page {page} of {pages}": "صفحة {page} من {pages}",
  "All categories": "كل الفئات",
  "All types": "كل الأنواع",
  "All packages": "كل الباقات",
  "All {n} services": "جميع الخدمات ({n})",
  Type: "النوع",
  Package: "الباقة",
  Fees: "الرسوم",
  Duration: "المدة",
  Anyone: "الجميع",
  None: "لا يوجد",
  "Working hours": "ساعات العمل",
  "Reference:": "الرقم المرجعي:",
  "No service matches those filters. Try clearing one of them, or":
    "لا توجد خدمة تطابق عوامل التصفية. جرّب مسح أحدها، أو",
  "Related services": "خدمات ذات صلة",
  "What you need": "ما تحتاجه",
  "Who it is for": "لمن هذه الخدمة",
  "Where to use it": "أين تستخدمها",
  "How you receive it": "كيف تستلمها",
  "Processed in {turnaround}": "تُنجَز خلال {turnaround}",
  "Takes about {turnaround} once submitted":
    "تستغرق نحو {turnaround} بعد التقديم",
  "Sign in to {action}": "سجّل الدخول لـ{action}",
  "No fee": "بدون رسوم",
  "{fee} payable on approval": "{fee} تُدفع عند الموافقة",
  "Open request {ref}": "فتح الطلب {ref}",
  "You already have this open — status: {status}":
    "لديك هذا الطلب مفتوحًا بالفعل — الحالة: {status}",
  "Quote reference": "اذكر الرقم المرجعي",
  "Request opened": "تم فتح الطلب",

  // --- forms ---------------------------------------------------------------
  "What would you like to share?": "بماذا تود أن تشاركنا؟",
  "Thank you — your {kind} has been recorded": "شكرًا لك — تم تسجيل {kind}",
  "Your {kind}": "{kind} الخاص بك",
  "A member of the team will be in touch by email. Reference numbers are issued once the service is connected.":
    "سيتواصل معك أحد أعضاء الفريق عبر البريد الإلكتروني. تُصدَر الأرقام المرجعية عند ربط الخدمة.",
  "Certificate, fine, permit…": "شهادة، مخالفة، تصريح…",
  "Emirates ID": "الهوية الإماراتية",
  "Emirates ID {id}": "الهوية الإماراتية {id}",
  "No account yet? Every Dubai Police service is listed on the":
    "ليس لديك حساب بعد؟ جميع خدمات شرطة دبي مدرجة في",
  ", and most can be started with UAE PASS alone.":
    "، ويمكن بدء معظمها بالهوية الرقمية UAE PASS وحدها.",
  "Opens the sample account for {name}.":
    "يفتح الحساب التجريبي الخاص بـ{name}.",
  "Sample account. The requests, fines and documents below are illustrative and live only in this browser.":
    "حساب تجريبي. الطلبات والمخالفات والمستندات أدناه توضيحية ولا توجد إلا في هذا المتصفح.",

  // --- portal copy ---------------------------------------------------------
  "One request needs something from you": "طلب واحد يحتاج إجراءً منك",
  "{n} requests need something from you": "{n} طلبات تحتاج إجراءً منك",
  "Anything you apply for shows up here with its reference number and status.":
    "كل ما تتقدم به يظهر هنا برقمه المرجعي وحالته.",
  "Anything you apply for shows up here with its reference number and its full history.":
    "كل ما تتقدم به يظهر هنا برقمه المرجعي وسجله الكامل.",
  "Try clearing the status filter or searching for the reference number instead.":
    "جرّب مسح تصفية الحالة أو البحث بالرقم المرجعي بدلًا من ذلك.",
  "Certificates, permits and receipts appear here as soon as a request is approved.":
    "تظهر الشهادات والتصاريح والإيصالات هنا فور اعتماد الطلب.",
  "Updates about your requests, fines and documents land here.":
    "تصل هنا مستجدات طلباتك ومخالفاتك ومستنداتك.",
  "No unpaid fines are recorded against this Emirates ID.":
    "لا توجد مخالفات غير مسددة مسجلة على هذه الهوية الإماراتية.",
  "Paying within {days} days of the issue date takes 25% off.":
    "السداد خلال {days} يومًا من تاريخ الإصدار يمنحك خصمًا بنسبة 25%.",
  "Pay now": "ادفع الآن",
  "Paying…": "جارٍ السداد…",
  "{n} black points": "{n} نقاط سوداء",
  "{ref} · issued {date}": "{ref} · صدر في {date}",
  "valid to {date}": "صالح حتى {date}",
  "{ref} · submitted {date} · {fee}": "{ref} · قُدّم في {date} · {fee}",

  // --- media ---------------------------------------------------------------
  "+{n} more": "+{n} أخرى",

  // --- page titles ---------------------------------------------------------
  "{name} | Dubai Police": "{name} | شرطة دبي",
  "{name} from Dubai Police.": "{name} من شرطة دبي.",
  "Contact Us | Dubai Police": "اتصل بنا | شرطة دبي",
  "Customer Centers | Dubai Police": "مراكز العملاء | شرطة دبي",
  "Customer Service Agreement | Dubai Police":
    "اتفاقية خدمة العملاء | شرطة دبي",
  "Documents | My Dubai Police": "المستندات | شرطة دبي الخاصة بي",
  "Fines | My Dubai Police": "المخالفات | شرطة دبي الخاصة بي",
  "Incidents | Dubai Police": "البلاغات | شرطة دبي",
  "Information | Dubai Police": "معلومات | شرطة دبي",
  "Laws & Legislation | Dubai Police": "القوانين والتشريعات | شرطة دبي",
  "My Requests | My Dubai Police": "طلباتي | شرطة دبي الخاصة بي",
  "Operations | Dubai Police": "العمليات | شرطة دبي",
  "Operations Console | Dubai Police": "منصة العمليات | شرطة دبي",
  "Overview | My Dubai Police": "نظرة عامة | شرطة دبي الخاصة بي",
  "Page not found | Dubai Police": "الصفحة غير موجودة | شرطة دبي",
  "Privacy Policy | Dubai Police": "سياسة الخصوصية | شرطة دبي",
  "Profile | My Dubai Police": "الملف الشخصي | شرطة دبي الخاصة بي",
  "Search | Dubai Police": "البحث | شرطة دبي",
  "Service not found | Dubai Police": "الخدمة غير موجودة | شرطة دبي",
  "Service requests | Dubai Police": "طلبات الخدمة | شرطة دبي",
  "Services | Dubai Police": "الخدمات | شرطة دبي",
  "Sign In | Dubai Police": "تسجيل الدخول | شرطة دبي",
  "Sitemap | Dubai Police": "خريطة الموقع | شرطة دبي",
  "Street Speed Limits | Dubai Police": "الحدود القصوى للسرعة | شرطة دبي",
  "Terms & Conditions | Dubai Police": "الشروط والأحكام | شرطة دبي",
  "Units | Dubai Police": "الوحدات | شرطة دبي",
  "Black Points of Traffic Violations | Dubai Police":
    "النقاط السوداء للمخالفات المرورية | شرطة دبي",

  // --- page descriptions ---------------------------------------------------
  "Laws and legislation, traffic black points, street speed limits, and sustainable development practices.":
    "القوانين والتشريعات، والنقاط السوداء للمخالفات المرورية، والحدود القصوى للسرعة، وممارسات التنمية المستدامة.",
  "Every Dubai Police service, with its fee, turnaround and who it is for.":
    "كل خدمة من خدمات شرطة دبي، مع رسومها ومدة إنجازها والفئة المستفيدة منها.",
  "Search Dubai Police services, news, events and information.":
    "ابحث في خدمات شرطة دبي وأخبارها وفعالياتها ومعلوماتها.",
  "Sign in with UAE PASS or your Dubai Police account to track applications and use personalised services.":
    "سجّل الدخول بالهوية الرقمية UAE PASS أو بحسابك لدى شرطة دبي لمتابعة طلباتك والاستفادة من خدمات مخصّصة.",
  "Track your requests, settle fines, download documents and manage your details.":
    "تابع طلباتك، وسدّد مخالفاتك، ونزّل مستنداتك، وأدر بياناتك.",
  "Incident queue, deployment and service requests.":
    "قائمة البلاغات والانتشار وطلبات الخدمة.",
  "Police stations and Smart Police Stations across Dubai, with addresses and opening hours.":
    "مراكز الشرطة ومراكز الشرطة الذكية في دبي، مع العناوين وساعات العمل.",
  "{total} centers · {smart} of them self-service Smart Police Stations":
    "{total} مركزًا · منها {smart} مراكز شرطة ذكية ذاتية الخدمة",
  "Fines, black points and vehicle confiscation periods for traffic violations in Dubai.":
    "الغرامات والنقاط السوداء ومدد حجز المركبات للمخالفات المرورية في دبي.",
  "Speed limits and radar control thresholds across Dubai's roads.":
    "الحدود القصوى للسرعة وحدود الضبط الراداري على طرق دبي.",
  "Dubai applies specific speed limits across its roads to ensure safety and smooth traffic flow. These limits vary depending on the type of road and location.":
    "تطبّق دبي حدودًا قصوى محددة للسرعة على طرقها لضمان السلامة وانسيابية الحركة المرورية. وتختلف هذه الحدود حسب نوع الطريق والموقع.",
  "How Dubai Police collects, uses, stores and protects the data you share on this website.":
    "كيف تجمع شرطة دبي البيانات التي تشاركها على هذا الموقع وتستخدمها وتخزّنها وتحميها.",
  "The terms that govern your use of the Dubai Police website and its electronic services.":
    "الشروط التي تحكم استخدامك لموقع شرطة دبي وخدماته الإلكترونية.",
  "Terms and conditions": "الشروط والأحكام",
  "What you can expect from Dubai Police at every counter, and what we ask of you in return.":
    "ما يمكنك توقعه من شرطة دبي عند كل نافذة خدمة، وما نطلبه منك في المقابل.",
  // --- services and account chrome ---------------------------------------
  "All {n} Dubai Police services, with the fee, the turnaround and who each one is for.": "جميع خدمات شرطة دبي البالغ عددها {n}، مع الرسوم ومدة الإنجاز والفئة المستفيدة من كل خدمة.",
  "All Services": "جميع الخدمات",
  "My Account": "حسابي",

  // --- list controls and site map ----------------------------------------
  "Newest first": "الأحدث أولاً",
  "Oldest first": "الأقدم أولاً",
  "A – Z": "أ – ي",
  "Main navigation": "التنقل الرئيسي",
  "All services": "جميع الخدمات",
  "Legal": "الشؤون القانونية",

  // --- result counts -----------------------------------------------------
  "{n} result for": "{n} نتيجة عن",
  "{n} results for": "{n} نتائج عن",
  "{n} suggestion for {q}": "{n} اقتراح عن {q}",
  "{n} suggestions for {q}": "{n} اقتراحات عن {q}",

  // --- partner logos ------------------------------------------------------
  "Innovation": "الابتكار",
  "Dubai Police AIX": "شرطة دبي AIX",
};
